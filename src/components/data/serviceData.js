import wash from "../../../public/service/wash.webp";
import ac from "../../../public/service/ac.webp";
import fridge from "../../../public/service/fridge.webp";
import chimney from "../../../public/service/chim.webp";
import oven from "../../../public/service/oven.webp";
import dish from "../../../public/service/dishwasher.webp";

const serviceData = {
  "washing-machine": {
    title: "Washing Machine",
    image: wash,
    ogImage: "/service/wash.webp",
    seoTitle: "Washing Machine Repair Chennai & Coimbatore | Frost Masters",
    seoDescription:
      "Expert washing machine repair in Chennai and Coimbatore. Top-load, front-load, drum, motor, PCB repair. Same-day service, genuine parts. Call for booking.",
    description:
      "Frost Masters offers professional washing machine repair and maintenance across Chennai and Coimbatore. Our technicians handle all major brands—LG, Samsung, Whirlpool, IFB, Bosch, and more—for both top-load and front-load models. From drum not spinning and water drainage issues to motor, belt, and PCB board repairs, we diagnose and fix the root cause. We also address water leakage, pipe blockages, vibration, unusual noise, door lock and sensor replacement, and provide periodic servicing and deep cleaning. Installation, uninstallation, and genuine spare parts with warranty are available. Book a visit and get a transparent estimate before any repair.",
    points: [
      "Diagnosis of all major brands and models",
      "Top-load and front-load repair solutions",
      "Drum not spinning or water not draining fix",
      "Motor, belt, and PCB board repairs",
      "Water leakage and pipe blockages",
      "Vibration and unusual noise troubleshooting",
      "Door lock and sensor replacement",
      "Periodic servicing and deep cleaning",
      "Installation and uninstallation support",
      "Genuine spare parts with warranty",
    ],
    faq: [
      {
        question: "Why is my washing machine not spinning?",
        answer:
          "Common causes include a faulty motor, worn belt, overloaded drum, or a defective lid/door switch. Our technician will inspect and fix the issue—often a belt replacement or motor repair resolves it.",
      },
      {
        question: "Do you repair front-load and top-load washing machines?",
        answer:
          "Yes. We service both front-load and top-load machines from all major brands including LG, Samsung, Whirlpool, IFB, and Bosch.",
      },
      {
        question: "What if water is not draining from the washing machine?",
        answer:
          "Blocked drain hose, clogged pump filter, or a faulty drain pump can cause this. We clean or replace the affected parts and restore normal drainage.",
      },
      {
        question: "Do you provide warranty on washing machine repairs?",
        answer:
          "Yes. We offer a 30-day service warranty on labour and use genuine or OEM spare parts with their manufacturer warranty where applicable.",
      },
    ],
    cityKeywords: [
      {
        city: "Chennai",
        keywords: [
          "washing machine repair Chennai",
          "washing machine service Chennai",
          "washing machine repair near me Chennai",
          "LG washing machine repair Chennai",
          "Samsung washing machine repair Chennai",
        ],
      },
      {
        city: "Coimbatore",
        keywords: [
          "washing machine repair Coimbatore",
          "washing machine service Coimbatore",
          "washing machine repair near me Coimbatore",
          "front load washing machine repair Coimbatore",
          "washing machine technician Coimbatore",
        ],
      },
    ],
  },
  refrigerator: {
    title: "Refrigerator",
    image: fridge,
    ogImage: "/service/fridge.webp",
    seoTitle:
      "Refrigerator Repair Chennai & Coimbatore | Fridge Repair | Frost Masters",
    seoDescription:
      "Refrigerator and fridge repair in Chennai and Coimbatore. Compressor, gas refill, cooling issues, thermostat. Same-day service, warranty on repairs. Call now.",
    description:
      "Get reliable refrigerator repair in Chennai and Coimbatore from Frost Masters. We fix single-door, double-door, and side-by-side fridges from brands like Samsung, LG, Whirlpool, Godrej, and more. Our services include compressor and gas refill, cooling issues, thermostat replacement, ice maker and freezer repairs, door seal and gasket replacement, and addressing noisy operation, water leakage, and defrost problems. We also repair interior lights, control panels, condenser coils, and fan motors. All repairs come with genuine parts and warranty support. Schedule a visit and get a clear estimate before we start any work.",
    points: [
      "Compressor and gas refill services",
      "Cooling issues and thermostat replacement",
      "Single and double-door fridge repair",
      "Ice maker and freezer not working fix",
      "Door seal and gasket replacement",
      "Noisy operation and vibration check",
      "Water leakage and defrost problems",
      "Interior light and control panel repair",
      "Condenser coil and fan motor servicing",
      "Genuine parts with warranty support",
    ],
    faq: [
      {
        question: "Why is my refrigerator not cooling enough?",
        answer:
          "It could be due to low gas, a faulty compressor, dirty condenser coils, or a defective thermostat. Our technician will diagnose and recommend the right fix—gas refill, compressor repair, or part replacement.",
      },
      {
        question: "Do you do gas refill for fridges?",
        answer:
          "Yes. We perform refrigerant (gas) refill and leak detection for refrigerators. We use the correct refrigerant type for your model.",
      },
      {
        question: "Can you repair double-door refrigerators?",
        answer:
          "Yes. We service single-door, double-door, and side-by-side refrigerators from all major brands.",
      },
      {
        question: "What causes water leakage from the fridge?",
        answer:
          "Often it’s a blocked defrost drain, damaged door gasket, or a faulty water tray. We clean or replace the parts as needed to stop the leakage.",
      },
    ],
    cityKeywords: [
      {
        city: "Chennai",
        keywords: [
          "refrigerator repair Chennai",
          "fridge repair Chennai",
          "refrigerator repair near me Chennai",
          "fridge gas refill Chennai",
          "double door fridge repair Chennai",
        ],
      },
      {
        city: "Coimbatore",
        keywords: [
          "refrigerator repair Coimbatore",
          "fridge repair Coimbatore",
          "refrigerator repair near me Coimbatore",
          "fridge cooling problem Coimbatore",
          "fridge compressor repair Coimbatore",
        ],
      },
    ],
  },
  "air-conditioner": {
    title: "Air Conditioner",
    image: ac,
    ogImage: "/service/ac.webp",
    seoTitle:
      "AC Repair Chennai & Coimbatore | Air Conditioner Repair | Frost Masters",
    seoDescription:
      "AC repair in Chennai and Coimbatore. Split & window AC, gas refill, compressor, cooling issues. Installation, AMC. Same-day service. Call for booking.",
    description:
      "Frost Masters provides expert air conditioner repair and maintenance in Chennai and Coimbatore. We service split ACs and window ACs from brands such as Daikin, Voltas, LG, Samsung, Blue Star, and Carrier. Our technicians handle gas leak detection and refilling, compressor and capacitor replacement, cooling and low airflow issues, water dripping and pipe blockages, remote sensor and display panel repair, fan motor and PCB replacement, and AC installation and uninstallation. We also offer annual maintenance and deep cleaning. All work is done with 100% genuine parts and a service guarantee. Book a visit and get a transparent quote before repair.",
    points: [
      "Split and window AC repair services",
      "Gas leak detection and refilling",
      "Compressor and capacitor replacement",
      "Cooling issues and low airflow fixes",
      "Water dripping and pipe blockage repair",
      "Remote sensor and display panel repair",
      "Fan motor and PCB board replacement",
      "AC installation and uninstallation",
      "Annual maintenance and deep cleaning",
      "100% genuine parts with service guarantee",
    ],
    faq: [
      {
        question: "Why is my AC not cooling properly?",
        answer:
          "Low gas, dirty filters, faulty compressor, or blocked condenser can cause poor cooling. We check refrigerant levels, clean filters and coils, and repair or replace faulty parts.",
      },
      {
        question: "Do you do AC gas refill?",
        answer:
          "Yes. We detect leaks and refill refrigerant (gas) for split and window ACs. We use the correct gas type as per the manufacturer specification.",
      },
      {
        question: "Do you service both split and window AC?",
        answer:
          "Yes. We repair and maintain both split and window ACs from all major brands.",
      },
      {
        question: "What causes water dripping from AC?",
        answer:
          "Blocked drain pipe, dirty drain pan, or improper slope can cause water dripping. We clean the drain line and fix the installation if needed.",
      },
    ],
    cityKeywords: [
      {
        city: "Chennai",
        keywords: [
          "AC repair Chennai",
          "air conditioner repair Chennai",
          "AC repair near me Chennai",
          "AC gas refill Chennai",
          "split AC repair Chennai",
        ],
      },
      {
        city: "Coimbatore",
        keywords: [
          "AC repair Coimbatore",
          "air conditioner repair Coimbatore",
          "AC repair near me Coimbatore",
          "AC gas refill Coimbatore",
          "window AC repair Coimbatore",
        ],
      },
    ],
  },
  // chimney: {
  //   title: "Chimney",
  //   image: chimney,
  //   ogImage: "/service/chim.webp",
  //   seoTitle:
  //     "Chimney Repair Chennai & Coimbatore | Kitchen Chimney Service | Frost Masters",
  //   seoDescription:
  //     "Chimney repair and service in Chennai and Coimbatore. Filter cleaning, motor repair, installation. All brands. Same-day service. Call for booking.",
  //   description:
  //     "Frost Masters offers chimney repair and maintenance in Chennai and Coimbatore. We service all major chimney brands and handle oil and grease filter deep cleaning, carbon filter replacement, motor and fan servicing, noisy operation and smoke issues, lighting and switch panel repair, auto-clean function repair, and exhaust duct inspection and cleaning. We also provide installation and wall-mount adjustments and annual maintenance or service contracts. Regular chimney servicing improves suction and extends the life of your appliance. Book a visit for a thorough check and transparent pricing.",
  //   points: [
  //     "Oil and grease filter deep cleaning",
  //     "Carbon filter replacement service",
  //     "Motor and fan servicing",
  //     "Noisy operation and smoke issues",
  //     "Lighting and switch panel fix",
  //     "Auto-clean function repair",
  //     "Exhaust duct inspection and cleaning",
  //     "Installation and wall-mount adjustments",
  //     "Annual maintenance and service contracts",
  //     "All major chimney brands supported",
  //   ],
  //   faq: [
  //     {
  //       question: "How often should I get my chimney serviced?",
  //       answer:
  //         "We recommend deep cleaning every 3–6 months for regular cooking. Heavy use may require more frequent servicing to maintain suction and avoid grease buildup.",
  //     },
  //     {
  //       question: "Why is my chimney making noise?",
  //       answer:
  //         "Dirty or damaged fan, loose parts, or worn motor bearings can cause noise. We clean or replace the affected parts to restore quiet operation.",
  //     },
  //     {
  //       question: "Do you replace chimney filters?",
  //       answer:
  //         "Yes. We replace oil and grease filters and carbon filters with compatible parts for your chimney model.",
  //     },
  //     {
  //       question: "Can you install a new chimney?",
  //       answer:
  //         "Yes. We provide chimney installation and wall-mount adjustments for new and existing setups.",
  //     },
  //   ],
  //   cityKeywords: [
  //     {
  //       city: "Chennai",
  //       keywords: [
  //         "chimney repair Chennai",
  //         "kitchen chimney service Chennai",
  //         "chimney cleaning Chennai",
  //         "chimney repair near me Chennai",
  //         "chimney installation Chennai",
  //       ],
  //     },
  //     {
  //       city: "Coimbatore",
  //       keywords: [
  //         "chimney repair Coimbatore",
  //         "kitchen chimney service Coimbatore",
  //         "chimney cleaning Coimbatore",
  //         "chimney repair near me Coimbatore",
  //         "chimney filter replacement Coimbatore",
  //       ],
  //     },
  //   ],
  // },
  // dishwasher: {
  //   title: "Dishwasher",
  //   image: dish,
  //   ogImage: "/service/dishwasher.webp",
  //   seoTitle: "Dishwasher Repair Chennai & Coimbatore | Frost Masters",
  //   seoDescription:
  //     "Dishwasher repair in Chennai and Coimbatore. Drainage, spray arms, pump, door seal. All brands. Same-day service. Call for booking.",
  //   description:
  //     "Frost Masters provides dishwasher repair and maintenance in Chennai and Coimbatore. We fix loading and draining issues, clean or replace spray arms and inlet valves, repair control panel and sensor issues, and address water leakage and blockages. We also repair racks, seals, door locks, noisy operation, poor cleaning, drain pump and motor replacement, and foul odor removal. We support all major brands and models and offer quick, reliable service with a guarantee. Book a visit and get a clear estimate before we proceed with any repair.",
  //   points: [
  //     "Loading or draining water issues",
  //     "Spray arms and inlet valve cleaning",
  //     "Control panel and sensor issues",
  //     "Water leakage and blockages fix",
  //     "Racks, seals, and door lock repair",
  //     "Noisy operation or poor cleaning fix",
  //     "Drain pump and motor replacement",
  //     "Foul odor removal and maintenance",
  //     "All brands and models supported",
  //     "Quick, reliable, and guaranteed service",
  //   ],
  //   faq: [
  //     {
  //       question: "Why is my dishwasher not draining?",
  //       answer:
  //         "Blocked drain hose, clogged filter, or a faulty drain pump can cause this. We clean the drain path and replace the pump if necessary.",
  //     },
  //     {
  //       question: "Do you repair all dishwasher brands?",
  //       answer:
  //         "Yes. We service dishwashers from Bosch, Siemens, IFB, LG, Samsung, and other major brands.",
  //     },
  //     {
  //       question: "What if the dishwasher is not cleaning properly?",
  //       answer:
  //         "Clogged spray arms, low water pressure, or a faulty inlet valve can reduce cleaning. We clean spray arms and repair or replace faulty parts.",
  //     },
  //     {
  //       question: "Can you fix dishwasher door seal leakage?",
  //       answer:
  //         "Yes. We replace door gaskets and seals to stop water leakage and ensure proper closing.",
  //     },
  //   ],
  //   cityKeywords: [
  //     {
  //       city: "Chennai",
  //       keywords: [
  //         "dishwasher repair Chennai",
  //         "dishwasher service Chennai",
  //         "dishwasher repair near me Chennai",
  //         "Bosch dishwasher repair Chennai",
  //         "IFB dishwasher repair Chennai",
  //       ],
  //     },
  //     {
  //       city: "Coimbatore",
  //       keywords: [
  //         "dishwasher repair Coimbatore",
  //         "dishwasher service Coimbatore",
  //         "dishwasher repair near me Coimbatore",
  //         "dishwasher not draining Coimbatore",
  //         "dishwasher repair technician Coimbatore",
  //       ],
  //     },
  //   ],
  // },
  "microwave-oven": {
    title: "Microwave Oven",
    image: oven,
    ogImage: "/service/oven.webp",
    seoTitle: "Microwave Oven Repair Chennai & Coimbatore | Frost Masters",
    seoDescription:
      "Microwave oven repair in Chennai and Coimbatore. No heating, magnetron, turntable, door latch. Solo, grill, convection. Same-day service. Call now.",
    description:
      "Frost Masters offers microwave oven repair in Chennai and Coimbatore. We fix no heating or uneven cooking, magnetron, diode, and capacitor repair, turntable motor and tray issues, display, buttons, and control panel problems, power supply and wiring checks, sparking or burning smell, door latch and sensor repairs, and interior and exterior deep cleaning. We service all types—solo, grill, and convection microwaves—from brands like Samsung, LG, IFB, and Panasonic, using genuine spare parts. Book a visit and get a transparent estimate before any repair.",
    points: [
      "No heating or uneven cooking fix",
      "Magnetron, diode, and capacitor repair",
      "Turntable motor and tray issues",
      "Display, buttons, and control panel fix",
      "Power supply and wiring checks",
      "Sparking or burning smell fix",
      "Door latch and sensor repairs",
      "Interior and exterior deep cleaning",
      "All types: solo, grill, convection",
      "Service with genuine spare parts",
    ],
    faq: [
      {
        question: "Why is my microwave not heating?",
        answer:
          "A faulty magnetron, blown high-voltage diode or capacitor, or a defective door switch can cause no heating. We test and replace the faulty component.",
      },
      {
        question: "Do you repair convection and grill microwaves?",
        answer:
          "Yes. We service solo, grill, and convection microwave ovens from all major brands.",
      },
      {
        question: "What causes sparking inside the microwave?",
        answer:
          "Metal objects, damaged waveguide cover, or food debris can cause sparking. We remove obstructions and replace the waveguide cover if damaged.",
      },
      {
        question: "Can you fix the turntable not rotating?",
        answer:
          "Yes. We repair or replace the turntable motor and tray so that food heats evenly.",
      },
    ],
    cityKeywords: [
      {
        city: "Chennai",
        keywords: [
          "microwave oven repair Chennai",
          "microwave repair Chennai",
          "microwave repair near me Chennai",
          "microwave not heating repair Chennai",
          "convection microwave repair Chennai",
        ],
      },
      {
        city: "Coimbatore",
        keywords: [
          "microwave oven repair Coimbatore",
          "microwave repair Coimbatore",
          "microwave repair near me Coimbatore",
          "microwave oven not working Coimbatore",
          "microwave repair technician Coimbatore",
        ],
      },
    ],
  },
};

export default serviceData;
